...
from mylib.figures import plot_skill_demand_over_time, plot_skill_income_waterfall, plot_state_demand
...
def input_page():
    if request.method == "POST":
        ...
        plot_skill_demand_over_time(jobpostDF=jobpostDF, skills_of_interest=top_skills, model=model, save_path=skill_demand_path)
        plot_state_demand(skills_of_interest=top_skills, model=model, jobpostDF=jobpostDF, shapefile_path="data/tl_2024_us_state.shp", save_path=state_demand_path)

        # Redirect to the analysis dashboard
        return redirect(url_for(
            "analysis_dashboard", 
            file_path=file_path,
            skill_demand_path=url_for('static', filename=f"fig/skill_demand_plot.png"),
            state_demand_path=url_for('static', filename=f"fig/state_demand_plot.png")
        ))
...
def analysis_dashboard():
    ...
    skill_demand_path = request.args.get('skill_demand_path', url_for('static', filename='fig/skill_demand_plot.png'))
    state_demand_path = request.args.get('state_demand_path', url_for('static', filename='fig/state_demand_plot.png'))
